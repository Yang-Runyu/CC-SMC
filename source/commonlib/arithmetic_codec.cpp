...
void Adaptive_Data_Model::copy(Adaptive_Data_Model & M, int forbid1, int forbid2)
{
	int mode = 1;
	if (forbid1 >= 0 && forbid2 >= 0) mode = 2;
	if(mode == 1) total_count = M.total_count - M.symbol_count[forbid1];
	else total_count = M.total_count - M.symbol_count[forbid1] - M.symbol_count[forbid2];
	unsigned j = 0;
	for (unsigned k = 0; k < M.data_symbols; k++)
	{
		if (k == forbid1 || k == forbid2) continue;
		symbol_count[j] = M.symbol_count[k];
		j++;
	}
	total_count--;
	update(false);
